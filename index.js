...
const context_listener = (event) => {
  ...
  window.api.showContextMenu(params);
};
...
function displayMovies(movies) {
  ...
  movies.forEach(async (m) => {
    ...
    a.addEventListener("click", async (evt) => {
      evt.preventDefault();
      await window.api.openPath(m.path);
    });
...
function openItemMenu(anchorEl, movie) {
  ...
  menu.querySelector(".open").addEventListener("click", async (e) => {
    await window.api.openPath(movie.path);
    closeMenu(menu);
  });
  menu.querySelector(".reveal").addEventListener("click", async (e) => {
    await window.api.revealInFolder(movie.path);
    closeMenu(menu);
  });
  menu.querySelector(".delete").addEventListener("click", (e) => {
    ...
    showConfirmModal({
      ...
      confirmText: "Delete",
      onConfirm: async () => {
        const res = await window.api.deletePath(movie.path);
        if (res && res.success) {
...
chooseBtn.addEventListener("click", async () => {
  ...
  showLoader();
  try {
    const movies = await window.api.getMovies(folder);
...
window.api.receive("inspect-element", (event) => {
